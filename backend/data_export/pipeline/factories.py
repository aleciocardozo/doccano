...
def create_writer(file_format: str, project: str = None) -> writers.Writer:
    mapping = {
...
def create_formatter(project: Project, file_format: str) -> List[Formatter]:
    ...
    is_triplet_extraction = getattr(project, "is_triplet_extraction", False)
    ...
    mapper_relation_extraction = {DATA: "text", Categories.column: "category"}
    ...
    mapping: Dict[str, Dict[str, List[Formatter]]] = {
        ...
        ProjectType.DOCUMENT_SENTIMENT_ANALYSIS: {
            CSV.name: [
                JoinedCategoryFormatter(Categories.column),
                JoinedCategoryFormatter(Comments.column),
                RenameFormatter(**mapper_text_classification),
        ...
        ProjectType.ASPECT_BASED_SENTIMENT_ANALYSIS: {
            JSONL.name: [
                DictFormatter(Spans.column),
                DictFormatter(Relations.column),
                DictFormatter(Comments.column),
...
def select_label_collection(project: Project) -> List[Type[Labels]]:
    ...
    mapping: Dict[str, List[Type[Labels]]] = {
        ...
        ProjectType.DOCUMENT_SENTIMENT_ANALYSIS: [Categories],
        ProjectType.ASPECT_BASED_SENTIMENT_ANALYSIS: [Spans, Relations] if is_triplet_extraction else [Categories, Spans, Relations]
    }
