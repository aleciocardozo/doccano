  ...
  <layout-text v-if="doc.id">
    ...
    <template #content>
      <v-card v-shortkey="shortKeys" @shortkey="addOrRemoveCategory">
        ...
        <v-divider v-if="isQuadrupleExtraction" />
        ...
        <v-expand-transition>
          <v-card-text v-show="showLabelTypes">
            <v-switch v-if="useRelationLabeling" v-model="relationMode">
              <template #label>
                <span v-if="relationMode">Relation (Polarity)</span>
                <span v-else>Span (Aspect/Opinion)</span>
              </template>
            </v-switch>
...
export default {
  ...
  computed: {
    ...
    isTripletExtraction() {
      return this.project.typeExtraction === 'triplet'
    },

    isQuadrupleExtraction() {
