<template>
  <v-card>
    <v-tabs v-if="hasMultiType" v-model="tab">
      ...
      <template v-else-if="isAspectBasedSentimentAnalysis && isQuadrupleExtraction">
        <v-tab class="text-capitalize">Category</v-tab>
        <v-tab class="text-capitalize">Span</v-tab>
  ...
  computed: {
    ...
    hasMultiType(): boolean {
      if ('projectType' in this.project) {
        return this.isIntentDetectionAndSlotFilling || this.isAspectBasedSentimentAnalysis || !!this.project.useRelation
      } else {
        return false
      }
    },
    ...
    isAspectBasedSentimentAnalysis(): boolean {
      return this.project.projectType === 'AspectBasedSentimentAnalysis'
    },

    isQuadrupleExtraction(): boolean {
      return this.project.typeExtraction === 'quadruple'
    },
    ...
    labelType(): string {
      if (this.hasMultiType) {
        ...
        } else if (this.isAspectBasedSentimentAnalysis && this.isQuadrupleExtraction) {
    ...
    service(): any {
      ...
      if (this.hasMultiType) {
        if (this.isIntentDetectionAndSlotFilling) {
          ...
          return [this.$services.categoryType, this.$services.spanType, this.$services.relationType][this.tab!]
        } else {
